...
async function main() {
  ...
  let transporter = nodemailer.createTransport({
    ...
    auth: {
      user: "shahpoojans18@gmail.com", // generated ethereal user
      pass: "BYm7jCA2t1zSMgkW", // generated ethereal password
    },
  });

  const data =  await User.find({});
  let variable= await Promise.all(data.map(async (user)=>{
    GeneratePDF(user);
    let info= await transporter.sendMail({
        ...
        attachments: [{
          ...
          path: 'D:/SGP(Main folder)/Backend(BuildCommunication)/'+user._id+'.pdf',
